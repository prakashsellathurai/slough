...
def write_text(x: float, y: float, text: str, font: tuple=FONT, color: str=TEXT_COLOR, align: str='left') -> None:
    t.penup()
    t.goto(x, y)
    t.pencolor(color)
...
def draw_vars_panel(vars_dict: dict[str, Any], step_idx: int, total_steps: int, event: str, func_name: str, return_value: Any | None) -> None:
...
def _is_simple_list(lst: list) -> bool:
    return len(lst) <= 20 and all((not isinstance(v, (list, dict)) for v in lst))
...
def draw_list_ds(lst: list, y_start: float) -> tuple[bool, float]:
    ...
    for i, val in enumerate(lst):
        ...
        write_text(bx + box_w / 2, y_start + box_h / 2 - 5, label, FONT_SMALL, TEXT_COLOR, 'center')
        write_text(bx + box_w / 2, y_start - 14, str(i), ('Consolas', 7, 'normal'), LINE_NUM_COLOR, 'center')
...
def draw_dict_ds(d: dict, y_start: float) -> tuple[bool, float]:
    ...
    for k, v in d.items():
        ...
        if len(label) > 70:
            label = label[:67] + '...'
        write_text(40, y_start - count * line_h, label, ('Consolas', 9, 'normal'), KEY_COLOR)
        count += 1
...
def draw_tree_ds(node: Any, y_start: float) -> tuple[bool, float]:
    ...
    h = _node_height(node)
    if h > 8:
        write_text(40, y_start, f'Tree ({h} levels) - too large to render', FONT, LINE_NUM_COLOR)
    ...
    if _is_linked_list(node):
        ...
        while n is not None and id(n) not in visited:
            ...
            write_text(x + node_w / 2, y + node_h / 2 - 5, label, FONT_SMALL, TEXT_COLOR, 'center')
            if hasattr(n, 'next') and n.next is not None:
                arrow_x = x + node_w + 2
...
def draw_data_structure(name: str, value: Any, y_start: float) -> tuple[bool, float]:
...
def draw_summary() -> None:
    ...
    for tc_data in STEPS:
        ...
        write_text(-400, y, f'  Test Case {STEPS.index(tc_data) + 1}:  return {rv!r}  expected {exp!r}  {status}', FONT, color)
        y -= 24
    write_text(0, 350, f'  {passed}/{total} tests passed', ('Consolas', 16, 'bold'), VAL_COLOR if passed == total else '#dc2626', 'center')
    write_text(0, -100, 'Click to exit', FONT, LINE_NUM_COLOR, 'center')
...
def animate() -> None:
    ...
    for tc_idx, tc_data in enumerate(STEPS):
        ...
        all_steps.append((tc_idx, {'lineno': final.get('lineno', 0), 'event': 'test_complete', 'func_name': '', 'vars': {}, 'return_value': tc_data['return_value']}))
    total = len(all_steps)
